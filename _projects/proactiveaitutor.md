...
교육공학과와 함께 진행한 7개월(2023.12 – 2024.06)의 연구 프로젝트로, 공동 연구자 1명과 함께 제1저자로 주도했습니다. LAK25에 *"Striking the Balance: Exploring Levels of AI Tutor Proactivity in Enhancing Online Self-Paced Learning"*으로 게재했습니다.

## 1. 리서치 배경

비대면·자기주도 학습이 빠르게 늘고 있지만, 이를 지원하는 챗봇 대부분은 단순 질의응답이나 FAQ 수준에 머물러 있습니다. 정작 학습을 가장 해치는 순간 — 이해도 저하, 흥미 감소, 참여 이탈 — 은 대개 겉으로 드러나지 않고, 학습자는 그 순간을 스스로 회복할 방법이 마땅치 않습니다. 그래서 '기다리는' 튜터가 아니라 '먼저 움직이는' 튜터가 이 문제를 바꿀 수 있을지 확인하고자 했습니다.
...
**접근.** 능동성(proactivity)을 켜짐/꺼짐의 이분법이 아니라 4단계 스펙트럼으로 모델링하고, 같은 튜터가 단계별로 다르게 행동하는 e-러닝 프로토타입(유튜브 강의 + 퀴즈; GPT API, FastAPI, HTML/CSS/JS)을 직접 제작했습니다:
...
**어떤 유저에게, 무엇을 확인하고 싶었나.** 자기주도 온라인 학습자(대학생)를 대상으로, *학습 성과*와 학습자의 *통제감* 사이에서 어떤 개입 수준이 가장 균형이 좋은지, 그리고 "더 능동적일수록 더 좋다"는 검증되지 않은 가정이 실제로 맞는지 확인하고자 했습니다.
...
- **결론.** 정답인 단일 개입 수준은 없습니다. 개입에는 효과와 부작용이 함께 따르므로 적절한 수준을 *동적으로* 조절해야 하며, 핵심 인사이트는 학습자별 로그를 분석해 실시간으로 개입 수준을 조정함으로써 특정 지표를 다른 지표의 희생 위에서 극대화하는 대신 모든 지표에서 우수한 결과를 유지하는 것입니다.

## 5. 결론
능동적 개입이 학생의 학습 경험과 성과에 좋은 영향을 미칠 것이라는 가설은 조건부 부정되었습니다. 본 연구에서의 파일럿 실험에서는 능동적 개입이 학습 성과(단기 퀴즈 점수)는 올렸지만, 학습 경험의 핵심 축인 '통제감'을 무너뜨렸고, 장기적으로 학습을 저해하는 '수동성(의존성)'을 유발한다는 부작용이 확인되었습니다.

## 6. 회고

- **표본 수.** 표본 및 설계의 일반화 한계. 피험자 내 설계(Within-subjects)에서 $n = 8$은 정량적 일반화를 하기에 작은 표본입니다. 실험 과정에서 역균형화(Counterbalancing)를 통해 순서 효과와 피로 효과를 최소화하려 노력했으나, 통계적 유의성을 완벽히 확보하기에는 한계가 있었습니다. 향후 과제에서는 더 크고 다양한 표본을 모집하여 피험자 간 설계(Between-subjects) 혹은 혼합 설계를 통해 각 개입 수준의 효과를 보다 깔끔하게 격리·검증하고자 합니다.
...
- **"AI 기능이 더 고도화되고 능동적일수록 유저에게 좋을 것이다"라는 생각은 착각일 수 있다는 점을 배웠습니다.** 또한 단순히 가설의 참/거짓을 알아내려고 했던 이 실험에서 "학습자의 자율성을 해치지 않는 선에서 동적으로 조절해야 한다"는 온라인 튜터 서비스의 새로운 UX 디자인 아이디어를 만나게 되었습니다. 어쩌면 UX 리서치는 가설에 대한 이분법적인 정답을 확인하는 과정이 아니라, 유저조차 인지하지 못한 '트레이드오프(Trade-off)'를 발견하고 제품이 나아가야 할 진짜 방향성을 제안하는 일이 아닐까요?
- 유저가 "만족한다"고 말하는 서베이 점수(자기 보고)와, 실제로 유저를 수동적으로 만드는 행동 데이터(질문 빈도 급감)가 충돌할 수 있음을 배웠습니다. 유저는 때로, 서비스를 만든 사람을 배려하고자 혹은 깊게 생각하지 않고 '만족함'에 점수를 주기도 하는 것 같습니다. **UX 리서처는 유저의 말(Saying)보다 행동(Doing)과 시스템 로그를 통해 다양한 각도로 평가해야함을 알게 되었습니다.**
</div>
